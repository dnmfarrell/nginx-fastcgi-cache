...
Returns a new Nginx::FastCGI::Cache object. Location is the only mandatory argument, and the directory must exist and be executable (aka readable) by the Perl process in order to be valid. The other two arguments accepted are levels and fastcgi_cache_key. These default to the standard nginx settings (see the L<nginx fastcgi documentation|http://nginx.org/en/docs/http/ngx_http_fastcgi_module.html>).
...
sub new {
    ...
    croak "unable to read location directory $args->{location} $!" unless -e $args->{location} && -x $args->{location};
    ...
    if (exists $args->{levels}) {
        ...
        if (ref $args->{levels} eq 'ARRAY'
            and @{$args->{levels}} > 0
            and @{$args->{levels}} < 4
            and @{$args->{levels}} == grep { $_ > 0 and $_ < 3 } @{$args->{levels}})
        {
    ...
    if (exists $args->{fastcgi_cache_key}) {
        ...
        if(ref $args->{fastcgi_cache_key} eq 'ARRAY'
           ...
           and @{$args->{fastcgi_cache_key}} == grep /scheme|request_method|host|request_uri/, @{$args->{fastcgi_cache_key}})
        {
        ...
        $self->{fastcgi_cache_key} = [ qw/scheme request_method host request_uri/ ];
    }
...
sub _purge_file {
    ...
    if (-e $path_to_purge and -w $path_to_purge) {
        unlink $path_to_purge or croak "unable to purge cache at $path_to_purge $!";
        return 1;
    }
